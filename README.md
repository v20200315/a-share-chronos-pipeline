...
- `market_data_pipeline`：已实现的行情数据采集、元数据校验、日线 K 线刷新流水线。
- `feature_pipeline`：特征工程流水线占位包。
- `model_pipeline`：模型训练与推理流水线占位包。
- `backtest_pipeline`：回测流水线占位包。

在仓库根目录运行已实现的行情数据 CLI：`python -m market_data_pipeline.cli`。

### 数据目录结构

`market_data_pipeline` 拥有 `data/market_data/` 下的原始行情数据输出。
`feature_pipeline` 应该把 `data/market_data/snapshots/latest/` 作为稳定输入契约。
